...
exports.index = async(req, res, next) => {
    ...
    if (!id || !chu_nen || !chu_ky) return res.jsonp({ error: 'missing data to launch the imaging program.' });

    if (id < 1 || id > 757) return res.jsonp({ error: 'Character ID not found' });
    ...
    try {
        ...
        res.jsonp({ error: 'Unable to process your request' });
    }
}